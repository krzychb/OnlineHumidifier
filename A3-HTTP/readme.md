...
HTTP binding is using URLs to issue commands. We got familiar with such commands in chapter [Operate the Socket from Web Browser](../7-Control#operate-the-socket-from-web-browser). For example, to operate humidifier on and off, we have been using URLs as follows:

* ``` http://192.168.1.114/humidifier/1 ``` - to turn humidifier on
* ``` http://192.168.1.114/humidifier/0 ``` - to turn it off

We will use similar concept to provide generic way to handle the commands as below:

| Command | URL |
| --- | --- |
| Turn humidifier on  | ``` http://192.168.1.114/hygrostat?Humidifier=ON ``` |
| Turn humidifier off | ``` http:// 192.168.1.114/hygrostat?Humidifier=OFF ``` |
| Turn auto mode on  | ``` http:// 192.168.1.114/hygrostat?AutoMode=ON ``` |
| Turn auto mode off | ``` http:// 192.168.1.114/hygrostat?AutoMode=OFF ``` |
| Change humidity set point to e.g. 38% | ``` http:// 192.168.1.114/hygrostat?HumiditySetPoint=38 ``` |


It turns out that sketch we have developed already has built in functionality to process such URLs. To check it out please open recently developed [OnlineHumidifier-openHAB.ino](../A2-openHAB/OnlineHumidifier-openHAB) and save it as *OnlineHumidifier-HTTP.ino*. Compile and upload it to ESP module. 
...
Using function ``` server.on("/hygrostat", []()  ``` that captures specific URI ``` /hygrostat ``` and basing on the above code, we are able retrieve the name and value pairs for each command. Then it is the matter of using particular name and value to operate our hygrostat. 

The function processing all three commands we need is provided below:
...
Now we should update function ``` showControlScreen() ``` that is serving the web page to provide all  commands / six URL discussed above.
...
HTTP item binding is similar to MQTT binding [discussed previously](../A2-openHAB/readme.md#items). The only practical difference is that instead of MQTT topic we are using an URL.  The two chapters below provide more details on that.


### Issuing Commands

What we need to do is replacement of MQTT binding with HTTP binding. All remaining information for each existing *item* stays the same.

The HTTP biding to switch humidifier on and off looks as follows:
...
That is all you need to get HTTP binding to work. Please see below complete [default.items](openhab/default.items) configuration file. 
...
Again remember to replace ```192.168.1.114 ``` with IP address of your module :smile:. Then upload it to openHAB *configurations/items* directory. The other configuration file, [default.sitemap](openhab/default.sitemap) is exactly the same as we have used [previously](../A2-openHAB#sitemap) setting up MQTT binding.
